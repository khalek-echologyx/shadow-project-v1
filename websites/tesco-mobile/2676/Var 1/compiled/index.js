(function pollForHansenPage() {
	if (
		...
		try {
			...
			if (
				!document.querySelector('body').classList.contains('EAOA-2676')
			) {
				document
					.querySelector('body')
					.insertAdjacentHTML(
				...
				$('#myBillsAndPaymentsTable').on(
					...
					function (event) {
						...
						var invoiceID = parent
							.find('.invoiceIDHiddenField')
							.val();
						var selectedBillType = parent
							.find('.billFormatDropDown')
							.val();
						...
						var url =
							selfCareShowInvoiceForm.prop('action') +
							'?' +
							params;
				...
				if (btnFlag === 'download') {
					...
					deleteBtnFlagCookie();
					document
						.querySelectorAll('#ViewBtn.small-button')[0]
						.click();
window.history.back();
