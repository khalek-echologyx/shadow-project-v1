(() => {
  ...
  function waitForElem(selector, callback, opts) {
    ...
    var minElements = typeof opts.minElements === "number" ? opts.minElements : 1;
  ...
  function poll(check, onSuccess, timeout, frequency) {
    timeout = typeof timeout === "number" ? timeout : 10000;
    frequency = typeof frequency === "number" ? frequency : 25;
    if (timeout <= 0) return;
    if (check()) { onSuccess(); return; }
    setTimeout(function () {
      poll(check, onSuccess, timeout - frequency, frequency);
    }, frequency);
  }
  ...
  function onRouteChange() {
    ...
    if (TARGET_PATH && path.indexOf(TARGET_PATH) === -1) {
      if (document.body) {
