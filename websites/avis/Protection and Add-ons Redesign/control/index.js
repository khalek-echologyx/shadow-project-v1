(function () {
    ...
    poll(
      ...
      function () {
        ...
        declineProtectionLabel.addEventListener("click", function (e) {
          ...
          if (noProtectionEl) {
            var isSelected = noProtectionEl.classList.contains('ancillaries-bundle--selected');
            ...
            if (isSelected) {
              setTimeout(function () {
                ...
                if (noProtectionEl.classList.contains('ancillaries-bundle--selected')) {
                  noProtectionEl.classList.remove('ancillaries-bundle--selected');
                  // Trigger update
                  checkState();
                }
              }, 50);
  ...
  function checkState() {
    if (window.jQuery) {
      ...
      var paidBundle = $('.ancillaries-bundle--selected').not('[data-code="No Protection"]').length > 0;

      // Sync Opt-Out Checkbox with No Protection Selection State
      var isNoProtectionSelected = $('.ancillaries-bundle--selected[data-code="No Protection"]').length > 0;
      $('#decline-protection-label input').prop('checked', isNoProtectionSelected);

      // Use Checkbox State for Button Logic (User Request)
      var declineChecked = $('#decline-protection-label input[type="checkbox"]').is(':checked');
      var anyBundle = $('.ancillaries-bundle--selected').length > 0; // Keeping for fallback/completeness
