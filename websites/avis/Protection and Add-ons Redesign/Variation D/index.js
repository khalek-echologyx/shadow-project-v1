(function () {
  ...
  window.updateAvisCarSummary = function () {
    ...
    section.innerHTML = '<p class="car-summary-title">Car Summary</p>' +
      ...
      '    <div class="accordion-header' + (taxAndFees.length > 0 ? ' active' : '') + '" data-has-items="' + (taxAndFees.length > 0) + '">' +
      ...
      '      <div class="accordion-header-icon-arrow" style="display: ' + (taxAndFees.length > 0 ? 'block' : 'none') + ';' + (taxAndFees.length > 0 ? ' transform: rotate(180deg); margin-top: 1px;' : '') + '">' + arrowDown + '</div>' +
      '     </div>' +
      '    </div>' +
      '    <div class="accordion-content"' + (taxAndFees.length > 0 ? ' style="max-height: 1000px;"' : '') + '>' +
