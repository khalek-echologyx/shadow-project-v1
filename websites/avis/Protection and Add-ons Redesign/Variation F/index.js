(function () {
  ...
  window.updateAvisCarSummary = function () {
    ...
    // Only manipulate the native bundles container when our custom section is
    // actually present in the DOM. If it isn't (e.g. the price/calculate XHR
    // fires before we've injected anything), leave the native container alone
    // so the page never goes blank.
    var customSectionPresent = !!document.getElementById(EXP_ID);
    if (originalBundlesContainer && customSectionPresent) {
      if (!hasProtectionBundles) {
    ...
    var amazonGiftCardAmount = priceCalc.amazonCashBack ? priceCalc.amazonCashBack.toFixed(2) : "0.00";
    ...
    var buildList = function (items) {
      ...
      return items.map(function (i) {
        ...
        if (i.code === 'GSO') {
          ...
          description += '<span style="display: block; font-size: 12px;">Est. ' + (priceCalc.currencyCode || "USD") + ' ' + gsoAmount + gsoSuffix + '</span>';
        }
        var itemStyle = i.code === 'GSO' ? ' style="align-items: center;"' : '';
        return '<div class="summary-item"' + itemStyle + '><span>' + description + '</span><span>' + itemAmount + '</span></div>';
      }).join('');
    };
    ...
    if (amazonGiftCardAmount !== "0.00" && amazonGiftCardAmount !== 0) {
      amazonHtml = '   <!-- amazon gift card alert -->' +
        ...
        '           <span class="amazon-reward-text">You will receive ' + formatPrice(currencySymbol, amazonGiftCardAmount) + '  on an Amazon.com Gift Card upon completing your rental!</span>' +
        '         </div>' +
    ...
    section.innerHTML = '<p class="car-summary-title">Car Summary</p>' +
      ...
      '      <span class="total-vehicle-rate-price-amount">' + formatPrice(currencySymbol, vehicleRate) + '</span> ' +
      '      <span class="total-vehicle-rate-price-save">' + formatPrice(currencySymbol, vehicleRateDiscount) + '</span>' +
      '     </div>' +
      '   </div>' +
      ...
      '      <div class="accordion-header-icon-price protection-add-ons-price">' + formatPrice(currencySymbol, protectionAndAddOnsTotal.toFixed(2)) + '</div>' +
      '      <div class="accordion-header-icon-arrow" style="display: ' + (combinedProtectionAddOns.length > 0 ? 'block' : 'none') + '; transform: rotate(0deg); margin-top: -9px;">' + arrowDown + '</div>' +
      '     </div>' +
      '    </div>' +
      ...
      '      <div class="summary-item"><span>Discount Code Savings</span><span>' + formatPrice(currencySymbol, (discountCodeSavings || "0.00")) + '</span></div>' +
      '      <div class="summary-item"><span>Pay Now Savings</span><span>' + formatPrice(currencySymbol, (payNowSaving || "0.00")) + '</span></div>' +
      '      <div class="summary-item"><span>Protection & Add-ons Savings</span><span>' + formatPrice(currencySymbol, (Number(protectionAndAddOnSavings).toFixed(2) || "0.00")) + '</span></div>' +
      '    </div>' +
      '  </div>' +
      ...
      '     <div class="accordion-header-icon">' +
      '      <div class="accordion-header-icon-price">' + formatPrice(currencySymbol, (taxAndFees.length > 0 ? taxAndFees.reduce(function (acc, item) { return acc + (item.amount || 0); }, 0).toFixed(2) : "0.00")) + '</div>' +
      '      <div class="accordion-header-icon-arrow" style="display: ' + (taxAndFees.length > 0 ? 'block' : 'none') + '; transform: rotate(0deg); margin-top: -9px;">' + arrowDown + '</div>' +
      '     </div>' +
      '    </div>' +
      '    <div class="accordion-content">' +
      taxAndFees.map(function (item) {
        return '<div class="summary-item"><a target="_blank" class="tax-fee-link" href="https://www.avis.com/en/customer-service/faqs/usa/fees-taxes#' + item.code + '">' + item.description + '</a><span>' + formatPrice(currencySymbol, (item.amount || 0).toFixed(2)) + '</span></div>';
      }).join('') +
      '    </div>' +
      '  </div>' +
      '   <div class="divider"></div>' +
      ...
      '     <span class="total-price">' + formatPrice(currencySymbol, (Number(price).toFixed(2) || "0.00")) + '</span>' +
      '  </div>' +
        ...
        try {
          if (isPriceCalc) {
            response.clone().json().then(function (data) {
              ...
              // Only call updateAvisCarSummary when our custom section is
              // already injected. If it isn't, the native page content must
              // not be touched — calling it too early is what caused the
              // page to go blank on the price/calculate XHR.
              if (window.updateAvisCarSummary && document.querySelector('.new-protection-section .car-summary-section')) {
                setTimeout(function () {
  ...
  function injectProtectionLayout() {
    ...
    for (var i = 0; i < protectionItems.length; i++) {
      ...
      if (newPriceEl && data.newPrice) {
        var symbol = getSymbol(window.__AVIS_PRICE_CALC__ ? window.__AVIS_PRICE_CALC__.currencyCode : "");
    ...
    // Add opt out section
    var optOutSectin = '<div class="opt-out-section" id="avis-opt-out-container">' +
    ...
    poll(
      ...
      function () {
        var targetSection = document.querySelector('.protection-container-grid');
  ...
  function runProtection() {
    // Parse residency synchronously from the URL immediately — this must
    // happen before injectProtectionLayout() runs so that residNotUSA is
    // already correct when the layout and state checks execute.
