import addOnItemsSvgObj from './addOnItemsSvgObj.js';
import { preferredPoint } from './preferredPoint.js';
...
(() => {
    ...
    if (protAndAddOnItemListEl) {
      ...
      protAndAddOnsItems.forEach((item) => {
        wrapper.insertAdjacentHTML(
          ...
          protAndAddOnItemUI(
            item.description,
            getPriceWithCurrenty(currencyCode, item.netSubtotal ? item.netSubtotal.toFixed(2) : 0),
            item,
            item.quantity
          )
        );
  ...
  async function runReviewAndBook() {
    ...
    var selectedItemForInitalSectionHTML = '<div class="protection-item ' + (selectedItemForInitalSection.freeCDWIndicator || Number(selectedItemForInitalSection.netSubtotal) === 0 ? 'included' : '') + '" data-code="' + (selectedItemForInitalSection.code || '') + '">'
      ...
      + '</div>'
      + '<div class="prot-details-content">'
      + '<p class="prot-details-content-text">' + selectedItemForInitalSection.description.html + '</p>'
      + '</div>'
      + '</div>';
    ...
    var protSection = '<div class="new-prot-bundle" id="' + TEST_ID + '">'
      ...
      + '<div class="intial-prot-cards">' + staticNoProtectionCard + (selectedItemForInitalSection ? selectedItemForInitalSectionHTML : '') + '</div>'
      ...
      staticNoProtCard.addEventListener("click", async (e) => {
        ...
        if (Object.keys(extrasBundle).length > 0) {
          selectedBundlePayload = {
            ...
            coverageRating: hasJsonBundle ? coverageRatings[jsonBundle.coverageRating] : 0,
            description: hasJsonBundle ? jsonBundle.bundleDescription.html : "",
            items: jsonBundleItems.length > 0 ? jsonBundleItems.map(item => {
              return {
                id: item.code || "",
                code: item.code || "",
                title: item.name || "",
