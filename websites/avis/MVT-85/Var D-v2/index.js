(() => {
  ...
  if (!window._mvt85Patched) {
    ...
    Node.prototype.replaceChild = function (newChild, oldChild) {
      if (oldChild && oldChild.parentNode !== this) {
        if (oldChild.parentNode) oldChild.parentNode.replaceChild(newChild, oldChild);
        return oldChild;
      }
  ...
  function updateAgeUI(targetElement, checkboxEl, selectedAge) {
    ...
    const selectEl = targetElement.querySelector("#mui-component-select-ageSelect");
    if (selectEl) {
      if (selectedAge === "25+") {
  ...
  function updateResidencyUI(targetElement, checkboxEl) {
    setTimeout(function () {
      ...
      const isUS = selectedResidency === "US" || selectedResidency === "USA" || selectedResidency === "I am a US resident";
  ...
  function updateWizardUI(targetElement, checkboxEl, wrapperEl, topRowEl) {
    ...
    if (text.includes("*")) {
      ...
      const wizardAppliedText = wrapperEl.querySelector('.mvt-85-wizard-applied-text');
  ...
  function updateDiscountUI(targetElement, checkboxEl, wrapperEl, topRowEl) {
    try {
      ...
      if (checkboxEl.checked) {
        ...
        const discountAppliedText = wrapperEl.querySelector('.mvt-85-discount-applied-text');
  ...
  function applyCode() {
    try {
      ...
      waitForStableElement(
        ...
        function (targetElement) {
          try {
            ...
            if (currentText !== "I am 25 or older" && !currentText.includes("25+")) {
            ...
            ageTextObserver.observe(selectEl, { childList: true, subtree: true, characterData: true });

            //add tooltip
            const toolTipUI = "<div class='mvt-85-tooltip'>" +
              infoSvg +
      ...
      waitForStableElement(
        ...
        function (targetElement) {
          try {
            ...
            const spans = targetElement.querySelectorAll("span");
            const isUS = spans.length >= 3 && (spans[2].textContent.trim() === "US" || spans[2].textContent.trim() === "USA" || spans[2].textContent.trim() === "I am a US resident");

            if (isUS) {
              if (spans.length >= 3 && spans[2].textContent !== "I am a US resident") {
                spans[2].textContent = "I am a US resident";
              }
              if (spans[0]) spans[0].style.display = "none";
              if (spans[1]) spans[1].style.display = "none";
            ...
            topRowEl.insertAdjacentHTML("beforeend", '<input type="checkbox" id="residencyCheckbox" class="mvt-85-checkbox" ' + checkedAttr + ' />');
            ...
            const toolTipEl = document.querySelector('[data-testid="residency-dropdown-button"] .mvt-85-tooltip');
            if (toolTipEl) {
      ...
      waitForStableElement('[data-testid="wizard-number-popup-trigger-button"]', function (targetElement) {
        try {
          if (!targetElement || !targetElement.parentNode) return;
          ...
          //add checkbox
          const checkedAttr = "checked";
          topRowEl.insertAdjacentHTML("beforeend", '<input type="checkbox" id="wizardCheckbox" class="mvt-85-checkbox" ' + checkedAttr + '/>');
          ...
          const toolTipEl = document.querySelector('[data-testid="wizard-number-popup-trigger-button"] .mvt-85-tooltip');
          if (toolTipEl) {
            toolTipEl.addEventListener('click', (e) => {
              e.stopPropagation();
            })
      ...
      waitForStableElement('[data-testid="discount-coupon-popup-trigger-button"]', function (targetElement) {
        try {
          if (!targetElement || !targetElement.parentNode) return;
          ...
          const toolTipEl = document.querySelector('[data-testid="discount-coupon-popup-trigger-button"] .mvt-85-tooltip');
            if (toolTipEl) {
              toolTipEl.addEventListener('click', (e) => {
                e.stopPropagation();
              })
  ...
  function startObserver() {
    ...
    const observer = new MutationObserver(function () {
      ...
      const residencyWrapperExists = document.querySelector("#residencyCheckbox");
      const residencyTargetExists = document.querySelector('[data-testid="residency-dropdown-button"]');
      const wizardWrapperExists = document.querySelector("#wizardCheckbox");
      const wizardTargetExists = document.querySelector('[data-testid="wizard-number-popup-trigger-button"]');
      const discountWrapperExists = document.querySelector("#discountCheckbox");
      const discountTargetExists = document.querySelector('[data-testid="discount-coupon-popup-trigger-button"]');
